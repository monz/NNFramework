function fitValue = nmse( Actual, Expected )
%NMSE Computes a fitValue measure for data.
% 
%   fitValue = nmse(Actual, Expected)
%   compares the data matrix 'Actual' with a reference value 'Expected' using
%   NMSE as a comparison yardstick to produce the number fitValue. fitValue is a
...
%    N = size(Actual,2). NMSE vary between -Inf (bad fit) to 1 (perfect fit);
%    a zero value for NMSE denotes that the data 'Actual' is no
...
%  'NMSE': fitValue(i) = 1 - (norm(Expted(:,i)-Actual(:,i)))/(norm(Expected(:,i)-mean(Expected(:,i))))^2
